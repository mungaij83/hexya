...
func (c BoolConditionField) EqualsFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.Equals(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) NotEqualsFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.NotEquals(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) GreaterFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.Greater(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) GreaterOrEqualFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.GreaterOrEqual(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) LowerFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.Lower(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) LowerOrEqualFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.LowerOrEqual(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) LikeFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.Like(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) ContainsFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.Contains(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) NotContainsFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.NotContains(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) IContainsFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.IContains(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) NotIContainsFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.NotIContains(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) ILikeFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.ILike(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) InFunc(arg func(loader.RecordSet) []bool) Condition {
	return Condition{
		Condition: c.ConditionField.In(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) NotInFunc(arg func(loader.RecordSet) []bool) Condition {
	return Condition{
		Condition: c.ConditionField.NotIn(arg),
	}
}
...
// The function will be evaluated when the query is performed and
// it will be given the RecordSet on which the query is made as parameter
func (c BoolConditionField) ChildOfFunc(arg func(loader.RecordSet) bool) Condition {
	return Condition{
		Condition: c.ConditionField.ChildOf(arg),
	}
}
